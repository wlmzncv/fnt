...
bool fnt::ullTofnt(unsigned long long x, unsigned int &real,
                   unsigned char &exp) {
  if (x <= maxR) {
    real = x;
    exp = 0;
...
bool fnt::normalize(int z, bool compulsory) {
  if (0 <= z && z <= maxE - 1) { // z值应该属于0~254
    ...
    if (z > exp) { // 要增大exp
      while (exp < z &&
             (!bitvec[0] ||
              compulsory)) { // 末尾不为1 除非compulsory为true(此时一定能达到z值
                             // 但可能会造成real值全为0)
        real >>= 1;
        bitvec >>= 1;
        ++exp;
      ...
      while (
          z < exp &&
          !bitvec[realLength - 1]) { // real的最高位不能为1
                                     // 否则会造成极大的数值扭曲 可能不会达到z值
        real <<= 1;
        bitvec <<= 1;
        --exp;
      }
...
bool operator!=(const fnt &v1, const fnt &v2) { return !(v1 == v2); }
bool operator>(const fnt &v1, const fnt &v2) {
  return !(v1 < v2) && !(v1 == v2);
}
bool operator<(const fnt &v1, const fnt &v2) {
  ...
  if (!v1.sign && v2.sign) {//v1为正 v2为负
    return false;
  }
  if (v1.sign && !v2.sign) { // v2为正 v1为负
    return true;
  }
  //v1与v2符号相同 判断两者绝对值的大小 true时v1大于v2
  bool greater = false;
  if (v1.exp == v2.exp && v1.real == v2.real) {
    return false;
  } else {
    //判断谁的绝对值更大 即v1/v2与1比较 real值更大的应在分子上
    unsigned int t;
    int s;
    if (v1.real > v2.real) {//v1/v2
      t = v1.real / v2.real;//分子
      s = v2.exp - v1.exp;//分母
      greater = t > (unsigned int)1 << s;
    } else { //v2/v1
      t = v2.real / v1.real;//分子
      s = v1.exp - v2.exp;//分母
      greater = t < (unsigned int)1 << s;
    }
    if(v1.sign){//负数
...
bool operator>=(const fnt &v1, const fnt &v2) { return !(v1 < v2); }
bool operator<=(const fnt &v1, const fnt &v2) { return v1 < v2 || v1 == v2; }
